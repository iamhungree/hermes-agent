...
def run_codex_stream(agent, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
    ...
    for attempt in range(max_stream_retries + 1):
        ...
        # Reset per attempt: on retry, stale deltas from a failed attempt must
        # not leak into the synthesis fallback (duplicate content) or suppress
        # delta streaming (stale has_tool_calls=True).
